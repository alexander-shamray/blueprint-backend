...
public class DatabaseSmokeTests(ServiceFixture fixture)
{
    ...
    public async Task A_transient_fault_does_not_double_apply_a_tracked_mutation()
    {
        ...
        {
            PaymentsDbContext seed = seedScope.ServiceProvider.GetRequiredService<PaymentsDbContext>();
    ...
    /// <summary>
    /// AddPaymentsInfrastructure over the fixture's database, with two changes
        ...
        services.AddPaymentsInfrastructure(new ConfigurationBuilder()
            .AddInMemoryCollection(
                new Dictionary<string, string?>
                {
                    ...
                    // multiplexer.
                    ["ConnectionStrings:RedisCache"] = "payments-redis.invalid:6379",
                    ["ConnectionStrings:RedisCoordination"] = "payments-redis.invalid:6380"
                })
            .Build());
    ...
    private static Task InsertProbeAsync(IUnitOfWork unitOfWork, Guid id, CancellationToken ct) =>
        unitOfWork.ExecuteRawAsync(
            "INSERT INTO payments.TransactionProbe (Id, Note) VALUES (@Id, @Note)",
