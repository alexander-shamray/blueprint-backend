...
public sealed class RetentionPurgeTests(ServiceFixture fixture) : IAsyncLifetime
{
    ...
    public async Task An_abandoned_outbox_row_survives_however_old_it_is()
    {
        ...
        await fixture.ExecuteAsync(
            "UPDATE inventory.OutboxMessages SET OccurredAt = {0} WHERE MessageId = {1};",
            LongAgo,
            poison.MessageId);
    ...
    public async Task Two_endpoints_differing_only_by_case_are_two_rows()
    {
        ...
        // survives.
        await fixture.StageInboxAsync(new InboxMessage(messageId, "inventory-orders", Recently));
        await fixture.StageInboxAsync(new InboxMessage(messageId, "inventory-Orders", Recently));
    ...
    public async Task Two_endpoints_differing_outside_the_code_page_are_two_rows()
    {
        ...
        await fixture.StageInboxAsync(new InboxMessage(messageId, "inventory-ж", Recently));
        await fixture.StageInboxAsync(new InboxMessage(messageId, "inventory-д", Recently));

        IReadOnlyList<InboxMessage> rows = await fixture.InboxAsync();

        rows.Count.ShouldBe(
            2,
            "varchar folds both endpoints to 'inventory-?', so the second insert is a " +
        ...
        rows.Select(r => r.Endpoint).ShouldBe(["inventory-ж", "inventory-д"], ignoreOrder: true);
    }
    ...
    public async Task An_inbox_row_is_purged_on_age_alone()
    {
        ...
        await fixture.StageInboxAsync(
            new InboxMessage(Guid.CreateVersion7(), "inventory-inventory-events", LongAgo),
            new InboxMessage(Guid.CreateVersion7(), "inventory-inventory-events", Recently));
    ...
    public async Task A_skewed_clock_purges_the_outbox_and_the_inbox_and_leaves_the_marker()
    {
        ...
        await fixture.StageInboxAsync(
            new InboxMessage(Guid.CreateVersion7(), "inventory-inventory-events", justNow));
