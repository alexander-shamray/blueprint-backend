...
/// </summary>
/// <remarks>
/// This suite declares its own endpoints and probe message rather than using
/// the real receive endpoint: two endpoints are what make the composite key a
/// claim, one consumer has to throw and another has to clear the change
/// tracker, and arranging that onto the real endpoint would be testing the
...
public sealed class InboxFilterTests(ServiceFixture fixture) : IAsyncLifetime
{
    ...
    /// to <c>Ordering.Infrastructure</c> and registering it here would need an
    ...
    {
        // The one path where this platform drops a message on purpose has to
        // carry a signal in §13, or an inbox hit reads like a genuine
        // redelivery from every dashboard. Homed here alone: InboxFilter<T> is
        // common code, so this is a property of the filter and not of either
        // service's wiring. The endpoint tag keeps a process-wide
        // MeterListener from reading a neighbouring suite's suppressions.
        await using ServiceProvider provider = BuildHost<FirstConsumer>();
        ITestHarness harness = provider.GetRequiredService<ITestHarness>();
        await harness.Start();
        ...
        // Scoped, because an unscoped wait can be satisfied by a row this test
        // did not write, and the second publish would then race the first
        // delivery's commit.
        await Eventually(() => fixture.InboxAsync(messageId), expected: 1);
        ...
        // Both deliveries recorded before the assertion, or the count would be
        // read while the redelivery was still in the pipe.
        await Eventually(
            () => Task.FromResult<IReadOnlyList<object>>(
                [.. harness.Consumed.Select<ProbeMessage>()]),
            expected: 2);
    ...
    public async Task The_filters_context_is_the_services_own_instance()
    {
        ...
        // so the assertion is about AddOrderingInfrastructure's registration.
    ...
    public async Task The_scoped_inbox_read_returns_only_the_message_it_was_asked_for()
    {
        ...
        // once per service and not common code.
