...
public sealed class ServiceFixture : IAsyncLifetime
{
        ...
        string? runtimeConnectionString = null)
    {
        string[] args =
        [
            .. Setting("ConnectionStrings:InventoryMigrator", migratorConnectionString),
            .. Setting("ConnectionStrings:Inventory", runtimeConnectionString)
        ];
    ...
    public Task ReplaceIdempotencyMarkerAsync(string key) =>
        ExecuteAsync(
            ...
            FROM inventory.IdempotencyMarkers
            WHERE [Key] = {0};

            DELETE FROM inventory.IdempotencyMarkers WHERE [Key] = {0};

            INSERT INTO inventory.IdempotencyMarkers ([Key], CommittedAt)
    ...
    public Task<int> ProbeRowCountAsync(Guid id) =>
        ScalarAsync<int>("SELECT Value = COUNT(*) FROM inventory.TransactionProbe WHERE Id = {0}", id);
