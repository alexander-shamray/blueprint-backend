...
suite writes itself, so no `gh` is needed. Each module is named for the
helper it tests, and its classes name the properties they check.
...
gate asserts, and why it needs both, is
