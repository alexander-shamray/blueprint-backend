...
`src/`, and the rollout job checks out `main` while the tag may name any
other revision. §15.2 tags both of a service's images with the commit they
were built from, so the rollout resolves the tag and reads that commit's
`src/`. It reads **only the workload being rolled** from it — the others
are running images built from other revisions — and it judges **each track
on its own image's probe routes**, taking the stable track's from the
release it is replacing. The ADR owns why; what does not move is the chart,
the thresholds and the gates themselves, which describe this rollout rather
than either image.
