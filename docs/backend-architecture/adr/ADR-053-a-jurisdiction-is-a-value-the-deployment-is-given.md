...
4. **What was said to a customer is kept as evidence, not as a copy.** A message
   ...
   notification's waiting work is marked terminal, as Shipping's is, and its
   contact row is still deleted. Every template is a **service message** about
   an order the customer placed; one line of promotion moves it into a class
   that needs a consent this platform records nowhere, so a template that is not
   about the customer's order is a new ADR before it is a pull request.
...
What the two countries ask, as far as an engineer can state it. **Every cell
...
round trip through every store and protocol a name or address crosses, and a
