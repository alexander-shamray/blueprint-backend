...
- **§3.2's Consumes cell gains `OrderCancelled`; §2.2's diagram gains
  Shipping's edges to Ordering and to the carrier; §11.5's table of realm
  objects and §15.4's inventory gain the client and the carrier's two
  keys.**
...
- **A failed row backs off and stays.** `Attempts` and `NextAttemptAt` on
  the row, the dispatcher's `2^min(Attempts, 8) × 5 s`. Nothing is
...
**Each outbound dependency, in §8.1's form:**
...
**Three instruments on a `Shipping.Carrier` meter**, in Payments' form:
...
past their first backoff, by state. §13.2's export names meters one by one,
...
- **The scaffold's suite**, in PR-1: the worker render's tree, and that the
  licence gate, the secret scan, the comment gate, the coverage filter and the
  architecture tests each enumerate the Shipping projects — a test whose subject
  is what each gate is looking at.
...
- **§15.4** gains the carrier's two rows in PR-2, and the client's in PR-4.
- **Appendix B** gains `Microsoft.Extensions.Http`'s row in PR-3a, and
  **§4.1's tree comment and `ServiceOptions`' remark** about the one host
  with credentials move in PR-3b.
- **§11.5's table of realm objects** gains `shipping-worker`, and
  **`docs/secrets.md`** its rotation row and its local-default row, in PR-4.
- **§3.2's Consumes cell** gains `OrderCancelled`, and **§2.2's diagram**
  the two edges, in PR-5.
- **§15.4's sentence that the solution has one options type, and its
  callout's close,** move in PR-6, which binds the second; the two windows
  join its inventory table there.
- **§13.6** gains the two rules and **§15.3** Shipping among the charts
  with no Redis, in PR-7.
