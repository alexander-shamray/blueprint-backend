...
| **D — docs or harness** | a new `/command`, a style pass, a chapter split, a runbook for an alert that already fires, a new gate, this contract | only the documents, harness, workflow, deploy or tools tree the issue names, `CLAUDE.md`, and the repository's own configuration files at the root — or, when no issue was filed, the paths the touch-set row declares. A new gate is its own `.github/<gate>/` directory with the README that owns its claim, the workflow that runs it, its entry in `repo-map.md`, its invocation in `docs/testing.md` and, when it enforces a chapter's rule, that chapter's one sentence naming it | inventories "while we are here"; code; a gate's claim restated outside its README |
